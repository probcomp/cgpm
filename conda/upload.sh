...
if [ -n "${TRAVIS_TAG}" ]; then
  ...
  if [ ${REAL_BRANCH} = "master" ]; then
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} ~/miniconda/conda-bld/linux-64/cgpm-*.tar.bz2 --force
  else
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} -l dev ~/miniconda/conda-bld/linux-64/cgpm-*.tar.bz2 --force
  fi
elif [ ${TRAVIS_BRANCH} = "master" ]; then
  if [ ${TRAVIS_EVENT_TYPE} = "cron" ]; then
    conda install anaconda-client
    anaconda -t ${CONDA_UPLOAD_TOKEN} upload -u ${CONDA_USER} -l nightly ~/miniconda/conda-bld/linux-64/cgpm-*.tar.bz2 --force
  else
    conda install anaconda-client
