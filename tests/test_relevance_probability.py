...
def check_posterior_score_answer(cgpm, answer, target, query, context):
    kwargs = dict(target=target, query=query)
...
def check_commutativity(cgpm, target, query, context):
    if isinstance(cgpm, View):
        assert np.allclose(
            cgpm.relevance_probability(target, query),
            cgpm.relevance_probability(query, target),
            atol=1e-4)
    
    elif isinstance(cgpm, (State, Engine)):
        assert np.allclose(
            cgpm.relevance_probability(target, query, context),
            cgpm.relevance_probability(query, target, context),
            atol=1e-3)
...
def check_agreement_view_state(view, state, target, query, context):
    assert (view.relevance_probability(target, query) ==
            state.relevance_probability(target, query, context))
...
def test_extreme_hypers_invariant_simple(cgpm, target, query):
    ...
    '''
    new_cgpm = tu.change_cluster_model_hyperparameters(cgpm, value=1e10)
    if isinstance(cgpm, View):
        assert np.allclose(
...
def test_extreme_crp_alpha_invariant_view(cgpm, target, query):
    ...
    and cluster assignments independent from variables.
    '''
    new_cgpm = tu.change_concentration_hyperparameters(cgpm, value=1e10)
    assert np.allclose(
...
# ----- TEST REPRODUCE ENCOUNTERED BUGS ----- #
def test_relevance_probability_is_less_than_one():
    ...
    # investigation led to the conclusion that rows (0, 1, 2, 3)
    # 'fit' cluster 0 better than rows (1, 2, 3). Must investigate further.
    assert cgpm.relevance_probability(target, query) <= 1.

def test_error_when_incorporating():
    cgpm = animals_engine.get_state(0)
    cgpm.relevance_probability(
        target={31: AsIs()}, query={32: AsIs()}, context=95)
