...
def test_marginalization(priorCGPM):
    ...
    if D == 1:
        ...
        summand_4 = [
            log_marginal_t(d.values()[0]) + log_conditional_t(dx2, d)
            for d in dx1]
        assert np.allclose(left_4, logsumexp(summand_4), atol=0.1)

        # check whether p(x2) is probability
        assert np.allclose(logsumexp([left_3, left_4]), 0, atol=0.1)
    ...
    # case 5: posterior cgpm, x_2 = [1] * D
        ...
        summand_5 = [
            log_marginal_t(d.values()[0]) + log_conditional_t(dx2, d)
            for d in dx1]
        assert np.allclose(left_5, logsumexp(summand_5), atol=0.1)
 
    else:
        assert False, "No test was called"

def test_bayes(priorCGPM):
    """
    ...
    if D == 1:  # CGPM with two columns
        ...
        assert np.allclose(left_3, right_3, atol=0.1)
        ...
        right_4 = (log_conditional_t(dx1[0], dx2) + log_marginal_t(dx2[0]) - 
                   log_marginal_t(dx1[0]))
        assert np.allclose(left_4, right_4, atol=0.1)
