...
def check_logpdf_multirow_answer(answer, query, evidence=None):
    view = initialize_view()
    
    l = view.logpdf_multirow(query, evidence, debug=True)
    assert np.allclose(answer, l)
...
def test_marginal_one_hypothetical_query_row():
    # P(x[1,0] = 1) = 7./12
    query = {1: {0: 1}}
    answer = np.log(7./12)
    check_logpdf_multirow_answer(answer, query)

def test_marginal_one_nonhypothetical_query_row():
    # P(x[0,0] = 1) = 2./3
    query = {0: {0: 1}}
    answer = np.log(1./2)
    check_logpdf_multirow_answer(answer, query)

def test_marginal_one_hypothetical_cluster():
    # P(Z[1] = 0) = 1./2
    query = {1: {Z: 0}}
    answer = np.log(1./2)
    check_logpdf_multirow_answer(answer, query)

def test_marginal_one_nonhypothetical_cluster():
    # P(Z[0] = 0) = 1.
    query = {0: {Z: 0}}
    answer = np.log(1)
    check_logpdf_multirow_answer(answer, query)
...
def test_joint_two_rows_one_column():
...
def test_joint_two_rows_two_columns_missing_values():
    # P(x[0,0] = 1, x[1,1]=1)
...
def test_joint_two_rows_with_clusters():
...
def test_conditional_one_column_one_row_given_another_row_second_part():
...
def test_conditional_two_columns_one_row_given_another_row_first_part():
...
def test_conditional_two_columns_one_row_given_another_row_second_part():
...
def test_quick_query_joint_logpdf_multirow_hypothetical():
    view = initialize_view()
    ...
    assert np.allclose(
        view.logpdf_multirow(deep_merged(row1, row2)),
        view.logpdf_multirow(deep_merged(row1_short, row2_short)))
...
def test_same_rowid_of_query_and_evidence_hypothetical():
    ...
    row1 = {1: {0: 1}}
    row2 = {2: {0: 1}}
    answer = view.logpdf_multirow(query=row2, evidence=row1)
    with pytest.warns(UserWarning):
        test_out = view.logpdf_multirow(query=row1.copy(), evidence=row1)
...
def test_same_rowid_of_query_and_evidence_nonhypothetical():
    ...
    row0 = {0: {0: 1}}
    row1 = {1: {0: 1}}
    answer = view.logpdf_multirow(query=row1, evidence=row0)
    with pytest.warns(UserWarning):
        test_out = view.logpdf_multirow(query=row0.copy(), evidence=row0)
    assert np.allclose(answer, test_out)
