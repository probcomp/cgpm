...
def _do_intialize(args):
    ...
    distargs = args[2]
    init_mode = args[3]
    S = state.State(X, cctypes, distargs=distargs)
    return S.get_metadata()
...
class Engine(object):
    ...
    def load_csv(self, filename, cctypes, distargs, num_states,
            init_mode='from prior'):
        ...
        args = [(self.X, self.cctypes, distargs, init_mode)
    ...
    def append_feature(self, X_f, cctype, distargs=None, ct_kernel=0, m=1,
        ...
        if cctype not in _cctypes_list:
            errmsg = "%s is an invalid cc_type. Valid types are: %s" % \
                (str(cctype), str(_cctypes_list))
    ...
    def transition(self, N=1, kernel_list=None, ct_kernel=2, which_rows=None,
            which_cols=None):
        """Do transitions."""
        args = [(N, self.X, kernel_list, ct_kernel, which_rows, which_cols,
